html_mid = '''
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset="UTF-8">
                <title>사마죄</title>
            </head>
            <body>
                <a href="/" style="display:inline-block; padding:6px 12px; background-color:#eee; border:1px solid #ccc; border-radius:4px; text-decoration:none; color:black;">처음으로 돌아가기</a>
                <h1 style="font-size:30px;text-align:center;">과도기</h1>
                <h2 style="font-size:20px;text-align:center;"> [Vocaloid] 하츠네 미쿠</h2>
                    <div style="
                            width:20%;
                            overflow:hidden;
                            border-radius:8px;
                            text-align:center;
                            margin:0 auto;
                        ">
                    <img 
                        src="/static/mid.png" 
                        alt="[VOCALOID] 하츠네미쿠" 
                        style="width:100%; height:100%; object-fit:cover; text-align:center;">
                    <figcaption style="font-size:12px; color:#666; text-align:center;">
                    이미지 출처 하츠네 미쿠 © Crypton Future Media, INC.py
                    </figcaption>
                    </div>

            </body>
            </html>
            '''